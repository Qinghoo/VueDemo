  ...
  <div class="page">
    ...
    <!-- <router-link to>
      <div class="empty-address">
        ...
        <nav-bar>
          <i slot="left" class="iconfont icon-back" @click="back"></i>
          <div slot="center">确认订单</div>
        </nav-bar>
    <div class="orders splitter">
      <div class="item" v-for="(good,key) in proOrderGoodList" :key="key">
        <img class="items-cover" :src="good.item.cover">
        <div class="item-title">{{good.item.name}}</div>
        <div class="price-info">
          <div class="item-old-line">
            <div class="item-old-price">￥{{good.item.priceNormalYuan}}</div>
          </div>
...
export default {
  ...
  created() {
      ...
      this.addGoodList=JSON.parse(window.localStorage.getItem('addGoodList'));
      this.total();
  },
  methods: {
    back(){
    ...
    submitOrder(){
      if(this.addGoodList!=null){
        for(var i=0;i<this.proOrderGoodList.length;i++){
            var index=GoodsTool.getKeyByItemId(this.addGoodList,this.proOrderGoodList[i].item.id);
